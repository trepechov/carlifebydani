...
3. **Website sync** — daily sync from Sheet to `wp_options`, with articles sorted by engagement: new articles (added today) first, then older articles ordered by click count descending, zero-click older articles last.
...
The plugin reuses this same infrastructure for the upcoming-session placeholder page. After each podcast, a new WP page is created manually to act as a placeholder for the next session. That page carries the same `news_csv` post meta, pointing to the active Google Sheet tab's public CSV export URL. `single.php` fetches and renders it identically to past episodes — no separate plugin page template or `wp_options` cache is needed for display. The plugin's role is to keep that metafield pointing at the right Sheet tab as sessions rotate.
...
| When | What |
|---|---|
| Daily 18:00 (site local time) | WP-Cron: **fetch GA4 clicks → update Sheet column G** → scrape → summarize → append → trim → **engagement-sorted sync** to `wp_options` |
| All week | Team curates directly in Google Sheets |
| Tuesday 23:00 (configurable) | WP-Cron: read storage → fetch article bodies → generate scripts → create Google Doc |
...
//           collection_interval='daily', recording_day='tue', recording_time='23:00',
...
// Session management (not part of the storage interface, used by admin / future AJAX):
...
create_session_sheet( string $date_name = '' ): bool|WP_Error  // add new tab + write header row; defaults to today
```
...
- Create tab: `POST /v4/spreadsheets/{id}:batchUpdate` with `addSheet` request

**Existing sheets migration:** tabs created before this change have columns A:F only. `read_data_rows()` treats missing G as `clicks=0` and missing H as `added_date=session_date` (best-effort fallback). `create_session_sheet()` writes headers `clicks` in G1 and `added_date` in H1 for new tabs.
...
**Daily cron invocation** calls `ENA_Analytics::fetch_clicks()` first (before `run()`), then `$this->storage->update_clicks()`, then `run()`. This is orchestrated in `ENA_Cron::run_daily_collection()` rather than inside `run()` so that `run()` remains reusable standalone (e.g. called from manual AJAX triggers or future orchestrators without re-fetching clicks).
...
```php
// Phase 2 orchestrator. Two responsibilities per run:
//
// 1. Engagement-sorted wp_options cache (powers template-live-news.php):
//    Reads all rows from storage, sorts them into three groups, writes JSON to ev_news_live_articles.
//    Sort order: [new today] → [older with clicks, highest first] → [older zero-click, last]
//    No API calls at render time — the template reads wp_options only.
//
// 2. Placeholder page news_csv meta update (powers single.php for the upcoming session page):
//    Keeps the configured placeholder page's news_csv post meta pointing at the active Sheet tab's
//    CSV export URL. Single.php fetches and renders it via card-article-external.php.
//    This is a no-op when the active tab hasn't changed since the last run.
//
// CSV export URL format (Sheet must be shared "Anyone with the link can view"):
//   https://docs.google.com/spreadsheets/d/{id}/gviz/tq?tqx=out:csv&sheet={tab_urlencoded}
//
// Depends on the storage adapter and ENA_Settings (injected in ENA_Plugin).

run(): array   // returns ['count' => int, 'page_id' => int|null, 'csv_url' => string|null]
```
...
// — Update placeholder page news_csv meta (if configured) —
$page_id = (int) $this->settings->get('placeholder_page_id')
if ($page_id) {
    $sheet_name = $this->storage->active_sheet_name()
    $sheet_id   = $this->settings->get('spreadsheet_id')
    $csv_url    = "https://docs.google.com/spreadsheets/d/{$sheet_id}/gviz/tq?tqx=out:csv&sheet=" . rawurlencode($sheet_name)
    update_post_meta($page_id, 'news_csv', $csv_url)
}
...
static deactivate(): void     // wp_clear_scheduled_hook for both hooks
static reschedule(): void     // called after settings save (recording day/time may change)
static register_hooks(): void // add_action for both cron hook names
static run_daily_collection(): void  // fetch GA4 clicks → update_clicks → ENA_Collector::run() → ENA_Sync::run()
static run_weekly_podcast(): void    // ENA_Podcast::run()
static add_intervals( array $schedules ): array  // adds 'ena_weekly' = WEEK_IN_SECONDS
```

Hook names: `ena_daily_collection` (configurable interval), `ena_weekly_podcast` (recording day/time local).
...
- `recording_day`: whitelisted to `mon|tue|wed|thu|fri|sat|sun`
- `recording_time`: validated `HH:MM` pattern
...
| Phase | How to verify |
|---|---|
| Activation | `wp cron event list` shows `ena_daily_collection` (next ~18:00) and `ena_weekly_podcast` (next Tuesday ~23:00) |
